...
def right_rotate(n, b):
    """Function to right rotate n by b bits"""
    return ((n >> b) | (n << (32 - b))) & 0xFFFFFFFF

def sha256(message):
    # Pre-processing: Padding the message
    message = bytearray(message, 'utf-8')  # Convert to bytearray
    orig_len_in_bits = (8 * len(message)) & 0xFFFFFFFFFFFFFFFF  # Original length in bits
...
print(f"SHA-256 Hash of '{text}' is: {hash_value}")
